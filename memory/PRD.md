...
- 2026-06 (fork): Пагинация дашборда. GET /posts теперь принимает page/page_size и возвращает {items,total,page,page_size,pages} (сортировка по дате, skip/limit). Dashboard грузит по 20 постов на страницу + кнопки «Назад/Вперёд» (data-testid pagination/prev/next), «Страница X из Y». Тесты обновлены под новый формат, pytest 69/69, e2e через UI (20 на стр.1, переход на стр.2). Telegraph ограничивает контент статьи ~64 КБ (≈400+ фото). При большом альбоме create_paginated_page/edit_paginated_page бьют статью на связанные страницы («часть 1/N» со ссылкой «➡️ Продолжение»), в канал уходит ссылка на первую. Порог MAX_HTML=40 КБ. Пост хранит photos_paths/videos_paths (списки). Применено и к легаси create_post. Проверено: 450 фото → 2 страницы, edit сохраняет пути, pytest 69/69.
- 2026-06 (fork): requirements.txt очищен. pip freeze в preview затянул внутренние пакеты (emergentintegrations, litellm) — их нет в публичном PyPI, деплой на VPS падал. Оставлен минимальный список реально импортируемых пакетов. update.sh теперь также копирует nginx-конфиг. Причина «зависаний»: запросы на загрузку не доходили до бэкенда (в логах только опрос GET) — nginx буферизует тело целиком (proxy_request_buffering on) и/или режет по client_max_body_size (413); у загрузчика не было таймаута → вис вечно. Фиксы: (1) UploadContext переписан — stall-таймаут 90с через AbortController, до 3 автоповторов с backoff, кнопка «Повторить» докачивает только незалитые слоты (nextIdx), «Убрать» скрывает job, понятные ошибки (413/тип/размер/обрыв). (2) Валидация слота устойчива к пустому content-type (fallback по расширению). (3) HEIC/HEIF — pillow-heif + watermark.ensure_web_format конвертит не-веб форматы в JPEG. (4) nginx: client_max_body_size 2100M + proxy_request_buffering off + таймауты 3600s. Проверено: HEIC→JPEG, wrong-type→400, e2e через UI, pytest 69/69. ВНИМАНИЕ: вступает в силу только после передеплоя на VPS (rebuild фронта + новый nginx + pip install pillow-heif).
